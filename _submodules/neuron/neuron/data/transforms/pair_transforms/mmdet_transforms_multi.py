...
__all__ = ['BasicPairTransforms_Multi', 'ExtraPairTransforms_Multi', 'TTFNetTransforms_Multi']
...
class _PairTransform(object):
    ...
    def __call__(self, item):
        ...
        item['img_z_ne'], item['img_meta_z_ne'], item['gt_bboxes_z_ne'] = \
            self._process_query(
                item['img_z_ne'],
                item['img_meta_z_ne'],
                item['gt_bboxes_z_ne'])
    ...
    def __call__(self, item):
        ...
        for i in range(10):
            ...
            item['img_x'], item['gt_bboxes_x'], mask_x = ops.random_crop(
                ...
                self.min_ious, self.min_scale)
            item['img_z_ne'], item['gt_bboxes_z_ne'], mask_x = ops.random_crop(
                item['img_z_ne'], item['gt_bboxes_z_ne'],
                self.min_ious, self.min_scale)
            mask = mask_z & mask_x
            if not mask.any():
                continue
            ...
            item['gt_bboxes_z_ne'] = item['gt_bboxes_z_ne'][mask]
            return item
        return item0
...
class BasicPairTransforms_Multi(Compose):
    ...
    def __init__(self,
        ...
        super(BasicPairTransforms_Multi, self).__init__(transforms=[
    ...
    def __call__(self, *args):
        ...
        if len(args) == 3:
            img_z, img_x, img_z_ne, target = args
            item = {
                'img_z': img_z,
                'img_x': img_x,
                'img_z_ne': img_z_ne,
                ...
                'img_meta_z_ne': {'ori_shape': img_z_ne.shape},
                'gt_bboxes_z': target['bboxes_z'],
                'gt_bboxes_x': target['bboxes_x'],
                'gt_bboxes_z_ne': target['bboxes_z_ne']}
            return super(BasicPairTransforms_Multi, self).__call__(item)
        else:
            return super(BasicPairTransforms_Multi, self).__call__(args[0])


@registry.register_module
class ExtraPairTransforms_Multi(Compose):
    ...
    def __init__(self,
        ...
        if with_basic:
            transforms += [BasicPairTransforms_Multi(**kwargs)]
        super(ExtraPairTransforms_Multi, self).__init__(transforms)
    
    def __call__(self, *args):
        assert len(args) in [1, 4], 'Invalid number of arguments'
        if len(args) == 4:
            img_z, img_x, img_z_ne, target = args
            item = {
                'img_z': img_z,
                ...
                'gt_bboxes_x': target['bboxes_x'],
                'gt_bboxes_z_ne': target['bboxes_z_ne']}
            return super(ExtraPairTransforms_Multi, self).__call__(item)
        else:
            return super(ExtraPairTransforms_Multi, self).__call__(args[0])


@registry.register_module
class TTFNetTransforms_Multi(Compose):
    ...
    def __init__(self,
        ...
        transforms += [
            ...
            BoundBoxes(),
            ToTensor()]
        super(TTFNetTransforms_Multi, self).__init__(transforms)
    ...
    def __call__(self, *args):
        ...
        if len(args) == 3:
            ...
            item = {
                ...
                'gt_bboxes_x': target['bboxes_x'],
                'gt_bboxes_z_ne': target['bboxes_z_ne']}
            return super(TTFNetTransforms_Multi, self).__call__(item)
        else:
            return super(TTFNetTransforms_Multi, self).__call__(args[0])
