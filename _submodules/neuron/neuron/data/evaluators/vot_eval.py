...
from _submodules.neuron.neuron.data.datasets import VOT
...
class VOT_Eval(Evaluator):
    ...
    def run_unsupervised(self, tracker, visualize=False):
        ...
        for s, (img_files, target) in enumerate(self.dataset):
            ...
            # tracking loop
            bboxes, times, scores = tracker.forward_test(
                img_files, anno_rects[0], anno_rects, visualize=visualize)   # by lc recall
            ...
            # record results
            # self._record(record_file, bboxes, times)
            self._record(record_file, bboxes, times, scores)  # for vot2018lt
    ...
    def _record(self, record_file, bboxes, times, scores):  # for vot18lt
        ...
        # otb_eval
        time_file = osp.join(record_file.replace('.txt', '_time.txt'))
        np.savetxt(time_file, times, fmt='%.8f')

        # convert scores to string
        lines = ['%.4f' % t for t in scores]
        lines = [t.replace('nan', 'NaN') for t in lines]

        # record running times
        print(record_file)
        score_file = '{}_confidence.value'.format(record_file.split('.')[0])
