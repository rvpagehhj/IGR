...
from _submodules.neuron.neuron.config import registry
from _submodules.neuron.neuron import ops
from collections import OrderedDict


__all__ = ['Seq2Pair', 'Image2Pair', 'Seq2Instance', 'Subset', 'Slice',
           'RandomConcat','Oneshot2Pair']
...
class Image2Pair(PairDataset):
    ...
    def __getitem__(self, index):
        ...
        # for cls
        # cls = target['labels'] - 1  # eg: array([ 6,  1, 27, 29,  1]) to array([ 5,  0, 26, 28,  0])
        ''' for joint training '''
        cls = target['labels']
        if len(bboxes) == 0:
            return self._random_next()
        ...
        img_x, bboxes_x = img.copy(), bboxes.copy()  # format: x1,y1,x2,y2

        # bound annotations by image boundaries
        h, w = img_z.shape[:2]
        bboxes_z = ops.bound_bboxes(bboxes_z, [w, h])  # format: x1,y1,x2,y2
        bboxes_x = ops.bound_bboxes(bboxes_x, [w, h])
        ...
        if self.transforms is not None:
            # return self.transforms(img_z, img_x, target)
            # for classifier
            # dict_ = self.transforms(img_z, img_x, target)
            # dict_.update({'cls':cls})
            # return dict_
            '''   for joint training  '''
            dict_ = self.transforms(img_z, img_x, target)
            dict_.update({'cls': cls})
...
@registry.register_module
class Oneshot2Pair(PairDataset):

    def __init__(self, imgs, transforms=None):
        super(Oneshot2Pair, self).__init__(
            name='{}_pairs'.format(imgs.name))  # imgs is RoiDataLoader  by lc
        self.imgs = imgs
        self.transforms = transforms
        # group images by aspect ratios (for sampling)
        self.group_flags = self._build_group_flags(imgs)

    ''' return blob, and then process blobs to biuld item'''
    # def __getitem__(self, index):
    #     img_z, img_x, target_z, target_x, cls = self.imgs[index]  # call the __getitem__ functions of RoiDataLoader
    #     # print(img_z.shape)
    #     # print(img_x.shape)
    #     bboxes_z = target_z['bboxes']
    #     bboxes_x = target_x['bboxes']
    #     if len(bboxes_z) == 0 or len(bboxes_x) == 0:
    #         return self._random_next()
    #
    #     # bound annotations by image boundaries
    #     h_z, w_z = img_z.shape[:2]
    #     h_x, w_x = img_x.shape[:2]
    #     bboxes_z = ops.bound_bboxes(bboxes_z, [w_z, h_z])
    #     bboxes_x = ops.bound_bboxes(bboxes_x, [w_x, h_x])
    #
    #     # build target
    #     target = {
    #         'bboxes_z': bboxes_z,
    ...
    #         # return self.transforms(img_z, img_x, target)
    #         dict_ = self.transforms(img_z, img_x, target)
    #         dict_.update({'cls': cls})  # is scalar not list
    #         return dict_
    #     else:
    #         return img_z, img_x, target

    def __getitem__(self, index):
        img_z, img_x, target_z, target_x, img_z_ne, target_z_ne, cls = self.imgs[index]  # call the __getitem__ functions of RoiDataLoader
        # print(img_z.shape)
        # print(img_x.shape)
        bboxes_z = target_z['bboxes']
        bboxes_x = target_x['bboxes']
        bboxes_z_ne = target_z_ne['bboxes']
        if len(bboxes_z) == 0 or len(bboxes_x) == 0 or len(bboxes_z_ne) == 0:
            return self._random_next()

        # bound annotations by image boundaries
        h_z, w_z = img_z.shape[:2]
        h_x, w_x = img_x.shape[:2]
        h_z_ne, w_z_ne = img_z_ne.shape[:2]
        bboxes_z = ops.bound_bboxes(bboxes_z, [w_z, h_z])
        bboxes_x = ops.bound_bboxes(bboxes_x, [w_x, h_x])
        bboxes_z_ne = ops.bound_bboxes(bboxes_z_ne, [w_z_ne, h_z_ne])

        # build target
        target = {
            'bboxes_z': bboxes_z,
            'bboxes_x': bboxes_x,
            'bboxes_z_ne': bboxes_z_ne}

        # apply transforms if applicable
        if self.transforms is not None:
            # return self.transforms(img_z, img_x, target)
            dict_ = self.transforms(img_z, img_x, img_z_ne, target)
            dict_.update({'cls': cls})  # is scalar not list
            return dict_
        else:
            return img_z, img_x, target
    ...
    def _build_group_flags(self, imgs):  # i think this is not required, so set to 0
        flags = np.zeros(len(imgs), dtype=np.uint8)
        # for i, name in enumerate(imgs.img_names):
        #     meta = imgs.img_dict[name]['target']['meta']
        #     if meta['width'] / meta['height'] > 1:
