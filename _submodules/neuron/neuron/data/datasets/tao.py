...
class Tao(SeqDataset):
    def __init__(self,root_dir=None, annotation_path=None, logger=None):
        ...
        super(Tao, self).__init__(
            self.name,
            root_dir=self.root_dir,max_distance=10)  # 300/30=10 by lc

    def _construct_seq_dict(self, root_dir, subset):
        # load subset sequence names
        split_file = osp.join(
            osp.dirname(__file__), 'train.json')
