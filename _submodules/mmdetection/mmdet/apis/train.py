...
def parse_losses(losses):
    ...
    log_vars['loss'] = loss
    for loss_name, loss_value in log_vars.items():  # 挨个赋给log_vars这个有序字典
...
def _dist_train(model,
    ...
    data_loaders = [
        build_dataloader(
            ...
            dist=True,
            seed=set_random_seed()) for ds in dataset
    ]
    # put model on gpus
...
def _non_dist_train(model,
    ...
    if cfg.resume_from:
        ...
        # runner.load_checkpoint(cfg.load_from_p)  # for joint after
        runner.load_checkpoint(cfg.load_from)
    runner.run(data_loaders, cfg.workflow, cfg.total_epochs)
