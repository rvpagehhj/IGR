...
class ATSSHead(AnchorHead):
    ...
    ''' by lc adding '''
    def loss_single_cls(self, cls_score,num_total_samples, cfg):
        device = cls_score[0].device
        cls_score = cls_score.permute(0, 2, 3,
                                      1).reshape(-1, self.cls_out_channels)
        labels = torch.zeros(cls_score.shape).to(device).long()
        label_weights = torch.ones(cls_score.shape).to(device)
        loss_cls = self.loss_cls(
            cls_score, labels, label_weights, avg_factor=num_total_samples)
        return loss_cls

    @force_fp32(apply_to=('cls_scores'))
    def loss_cls_score(self,
             cls_scores, cfg ):
        losses_cls = []
        for ss in range(len(cls_scores)):
            losses_cls.append(self.loss_single_cls(cls_scores[ss],
                num_total_samples=1,
                cfg = cfg))
        return dict(
            loss_cls=losses_cls )
    ...
    def loss(self,
        ...
        losses_cls, losses_bbox, loss_centerness,\
            bbox_avg_factor = multi_apply(
                ...
                cfg=cfg)     # 输出的应该是一个打包的list

        # print('一个实例结束啦')
        bbox_avg_factor = sum(bbox_avg_factor)
        bbox_avg_factor = reduce_mean(bbox_avg_factor).item()
        bbox_avg_factor = bbox_avg_factor + 1e-6   # by lc 避免出现0为除数
    ...
    def centerness_target(self, anchors, bbox_targets):
        ...
        centerness = torch.nan_to_num(centerness, nan=0.1)
